...
|--------|----------------|
| Account & Sessions | Public key (masked) Â· active web sessions Â· revoke session Â· mobile app link status |
| Notifications | Toggles per notification type: trade matches Â· escrow funded Â· payment sent Â· disputes Â· price alerts |
...
| About Peach | Version number Â· licenses Â· legal info Â· links |
...
- **Clipboard copy not working in preview** — `navigator.clipboard` and the `execCommand` fallback both fail in the Claude.ai iframe context. Works in production (HTTPS + no iframe restriction). No fix needed for prod, but preview is broken.
